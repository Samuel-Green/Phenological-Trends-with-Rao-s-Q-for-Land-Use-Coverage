...
## Import the shape file of land cover classification
...
if (crs(MS_PPI_Timeseries) != crs(MacchiaSacra_LandCover_Vector)){
  message("The shapefile's CRS differs from the diversity index raster's CRS")
  MacchiaSacra_LandCover_Vector <- project(MacchiaSacra_LandCover_Vector, crs(MS_PPI_Timeseries)) # reprojects the shapefile if CRSs differ
...
message(paste("Calculating final Rao's Q with optimized \U03B1 (", MS.PPI.Optimal_Alpha, ")...", sep=""))
